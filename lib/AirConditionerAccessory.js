...
class AirConditionerAccessory extends BaseAccessory {
	...
	constructor(...props) {
		...
		this.device.context.noSwing = true
	}

	_registerPlatformAccessory() {
		const {Service} = this.hap;

		this.accessory.context = { lastAcMode: this.cmdAuto };
	...
	_registerCharacteristics(dps) {
		...
		if (!this.device.context.noFan && !fanService)
			fanService = this.accessory.addService(Service.Fanv2, this.device.context.name + ' Fan');
		else if (this.device.context.noFan && fanService) {
			this.accessory.removeService(fanService);
			fanService = null;
		}


		if (!this.device.context.noDry && !dryService)
			dryService = this.accessory.addService(Service.HumidifierDehumidifier, this.device.context.name + ' Dry');
		else if (this.device.context.noDry && dryService) {
		...
		const characteristicActive = service.getCharacteristic(Characteristic.Active)
			...
			.onGet(this._promiseGet(this.getActive))
			.onSet(this._promiseSet(this.setActive));
		...
		const characteristicCurrentHeaterCoolerState = service.getCharacteristic(Characteristic.CurrentHeaterCoolerState)
			...
			.onGet(this._promiseGet(this.getCurrentHeaterCoolerState));
		...
		const characteristicTargetHeaterCoolerState = service.getCharacteristic(Characteristic.TargetHeaterCoolerState)
			...
			.onGet(this._promiseGet(this.getTargetHeaterCoolerState))
			.onSet(this._promiseSet(this.setTargetHeaterCoolerState));

		const characteristicCurrentTemperature = service.getCharacteristic(Characteristic.CurrentTemperature)
			.updateValue(dps['3']/10)
			.onGet(this._promiseGet(this.getCurrentTemperature));
		...
		if (!this.device.context.noSwing) {
			characteristicSwingMode = service.getCharacteristic(Characteristic.SwingMode)
				...
				.onGet(this._promiseGet(this.getSwingMode))
				.onSet(this._promiseSet(this.setSwingMode));
		...
		if (!this.device.context.noCool) {
			characteristicCoolingThresholdTemperature = service.getCharacteristic(Characteristic.CoolingThresholdTemperature)
				...
				.updateValue(dps['2'])
				.onGet(this._promiseGet(this.getState.bind(this, '2')))
				.onSet(this._promiseSet(this.setTargetThresholdTemperature.bind(this, 'cool')));
		...
		if (!this.device.context.noHeat) {
			characteristicHeatingThresholdTemperature = service.getCharacteristic(Characteristic.HeatingThresholdTemperature)
				...
				.updateValue(dps['2'])
				.onGet(this._promiseGet(this.getState.bind(this, '2')))
				.onSet(this._promiseSet(this.setTargetThresholdTemperature.bind(this, 'heat')));
		...
		if (!this.device.context.noFanSpeed) {
			characteristicRotationSpeed = service.getCharacteristic(Characteristic.RotationSpeed)
				...
				.onGet(this._promiseGet(this.getRotationSpeed))
				.onSet(this._promiseSet(this.setRotationSpeed));
		...
		let fanCharacteristicRotationSpeed, fanCharacteristicSwingMode, fanCharacteristicActive;
		if (fanService) {
			fanCharacteristicActive = fanService.getCharacteristic(Characteristic.Active)
				.updateValue(this._getFanActive(dps))
				.onGet(this._promiseGet(this.getFanActive))
				.onSet(this._promiseSet(this.setFanActive));
		...
		let dryCharacteristicRotationSpeed ,dryCharacteristicSwingMode, dryCharacteristicCurrentHumidifierDehumidifierState, dryCharacteristicActive;
		if (dryService) {
			dryCharacteristicActive = dryService.getCharacteristic(Characteristic.Active)
				.updateValue(this._getFanActive(dps))
				.onGet(this._promiseGet(this.getDryActive))
				.onSet(this._promiseSet(this.setDryActive));
			...
			dryCharacteristicCurrentHumidifierDehumidifierState = dryService.getCharacteristic(Characteristic.CurrentHumidifierDehumidifierState)
				.updateValue(this._getCurrentHumidifierDehumidifierState(dps))
				.onGet(this._promiseGet(this.getCurrentHumidifierDehumidifierState));
		...
		this.device.on('change', (changes, state) => {
			if ('1' in changes || '4' in changes) {
				...
				if (dryService) {
					...
					dryCharacteristicCurrentHumidifierDehumidifierState.updateValue(this._getCurrentHumidifierDehumidifierState(state))
				}
			}
			...
			if ('3' in changes && characteristicCurrentTemperature.value !== changes['3']) characteristicCurrentTemperature.updateValue(changes['3']/10);
			...
			if ('104' && !this.device.context.noSwing) {
	...
	_getActive(dps) {
		...
		if (!dps || !dps['1']) return Characteristic.CurrentHeaterCoolerState.INACTIVE;
	...
	setActive(value, callback) {
		...
		switch (value) {
			case Characteristic.Active.ACTIVE:
				return this.setMultiState({'4': this.accessory.context.lastAcMode, '1': true}, callback);
	...
	_getCurrentHeaterCoolerState(dps) {
		...
		switch (dps['4']) {
			case this.cmdCool:
				this.accessory.context.lastAcMode = this.cmdCool;
				return Characteristic.CurrentHeaterCoolerState.COOLING;

			case this.cmdHeat:
				this.accessory.context.lastAcMode = this.cmdHeat
	...
	_getRotationSpeed(dps) {
		if (!dps || dps['5'] === undefined || dps['5'] === null) {
			return this._hkRotationSpeed || 0;
		}
	...
	_getFanActive(dps) {
		const {Characteristic} = this.hap;
		if (!dps || !dps['1'] || dps['4'] !== this.cmdFan) return Characteristic.Active.INACTIVE;
		return Characteristic.Active.ACTIVE;
	}
	...
	_getDryActive(dps) {
		const {Characteristic} = this.hap;
		if (!dps || !dps['1'] || dps['4'] !== this.cmdDry) return Characteristic.Active.INACTIVE;
		return Characteristic.Active.ACTIVE;
	}
	...
	_getCurrentHumidifierDehumidifierState(dps) {
		const {Characteristic} = this.hap;
		if (!dps || !dps['1'] || dps['4'] !== this.cmdDry) return Characteristic.CurrentHumidifierDehumidifierState.INACTIVE;
