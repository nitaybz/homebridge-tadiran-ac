...
class TuyaAccessory extends EventEmitter {
	constructor(props) {
		...
		if (!(props.id && props.key && props.ip) && !props.fake) return console.log('[TadiranAC] Insufficient details to initialize:', props);
		...
		if (this.context.version >= 3.2) {
			...
			console.log(`[TadiranAC] Changing ping gap for ${this.context.name} to ${this.context.pingGap}s`);
		}
	...
	_connect() {
		...
		this._socket.on('error', err => {
			this.connected = false;
			console.log(`[TadiranAC] Socket had a problem and will reconnect to ${this.context.name} (${err && err.code || err})`);
			...
			if (err) {
				if (err.code === 'ENOBUFS') {
					console.warn('[TadiranAC] Operating system complained of resource exhaustion; did I open too many sockets?');
					console.log('[TadiranAC] Slowing down retry attempts; if you see this happening often, it could mean some sort of incompatibility.');
					delay = 60000;
		...
		this._socket.on('close', () => {
			this.connected = false;
			//console.log('[TadiranAC] Closed connection with', this.context.name);
		});

		this._socket.on('end', () => {
			this.connected = false;
			console.log('[TadiranAC] Disconnected from', this.context.name);
		});
	}
	...
	_msgHandler_3_1(task, callback) {
		...
		if (this.context.intro === false && cmd !== 9)
			console.log('[TadiranAC] Message from', this.context.name + ':', data);
		...
		switch (cmd) {
			...
			case 8: {
				...
				try {
					...
					this._logOddMessageOnce(cmd, task.msg);
					break;
				}

				if (data && data.dps) {
					//console.log('[TadiranAC] Update from', this.context.name, 'with command', cmd + ':', data.dps);
			...
			case 10:
				if (data) {
					if (data === 'json obj data unvalid') {
						console.log(`[TadiranAC] ${this.context.name} (${this.context.version}) didn't respond with its current state.`);
						this.emit('change', {}, this.state);
						break;
					}
	...
	_msgHandler_3_3(task, callback) {
		...
		switch (cmd) {
			...
			case 10:
				if (data) {
					if (data.dps) {
						// console.log(`[TadiranAC] Heard back from ${this.context.name} with command ${cmd}`);
						this._change(data.dps);
					} else {
						this._logOddMessageOnce(cmd, task.msg);
					}
	...
	update(o) {
		...
		let result = false;
		if (hasDataPoint) {
			console.log("[TadiranAC] Sending", this.context.name, JSON.stringify(dps));
			result = this._send({
				data: {
			...
			if (result !== true) console.log("[TadiranAC] Result", result);
			if (this.context.sendEmptyUpdate) {
				console.log("[TadiranAC] Sending", this.context.name, 'empty signature');
				this._send({cmd: 7});
			}
		} else {
			console.log(`[TadiranAC] Sending first query to ${this.context.name} (${this.context.version})`);
			result = this._send({
				data: {
	...
	// Log "odd / malformed" Tuya frames once per (cmd, version) pair per device
	// instead of every poll cycle. Earlier versions would also dump the binary
	// `data` / `decryptedMsg` directly, which printed as garbled bytes in logs.
	// We now log only the hex of the raw frame, and only the first time we see
	// that command code, so logs are useful but not spammy. Closes #7.
	_logOddMessageOnce(cmd, msg) {
		this._loggedOddCmds = this._loggedOddCmds || new Set();
		if (this._loggedOddCmds.has(cmd)) return;
		this._loggedOddCmds.add(cmd);
		const rawHex = msg && Buffer.isBuffer(msg) ? msg.toString('hex') : String(msg);
		console.log(`[TadiranAC] Odd / malformed frame from ${this.context.name} (v${this.context.version}) cmd ${cmd}, raw hex: ${rawHex}`);
		console.log(`[TadiranAC] Suppressing further "odd message" logs for cmd ${cmd} on ${this.context.name}.`);
	}
	...
	_fakeUpdate(dps) {
		console.log('[TadiranAC] Fake update:', JSON.stringify(dps));
