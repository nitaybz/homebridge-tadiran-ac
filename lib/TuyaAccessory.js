...
class TuyaAccessory extends EventEmitter {
		...
		switch (cmd) {
			...
			case 8: {
				...
				try {
					...
					data = decryptedMsg;
					console.log(`[TadiranAC] Odd message from ${this.context.name} with command ${cmd}:`, data);
					console.log(`[TadiranAC] Raw message from ${this.context.name} (${this.context.version}) with command ${cmd}:`, task.msg.toString('hex'));
					break;
				}
			...
			case 10:
				if (data) {
					...
					try {
						...
						console.log(`[TadiranAC] Malformed update from ${this.context.name} with command ${cmd}:`, data);
						console.log(`[TadiranAC] Raw update from ${this.context.name} (${this.context.version}) with command ${cmd}:`, task.msg.toString('hex'));
						break;
					}
	...
	_msgHandler_3_3(task, callback) {
		...
		try {
			...
			console.log(`[TadiranAC] Odd message from ${this.context.name} with command ${cmd}:`, decryptedMsg);
			console.log(`[TadiranAC] Raw message from ${this.context.name} (${this.context.version}) with command ${cmd}:`, task.msg.toString('hex'));
			return callback();
		}
			...
			case 10:
				if (data) {
					if (data.dps) {
						...
						console.log(`[TadiranAC] Malformed message from ${this.context.name} with command ${cmd}:`, decryptedMsg);
						console.log(`[TadiranAC] Raw message from ${this.context.name} (${this.context.version}) with command ${cmd}:`, task.msg.toString('hex'));
					}
				}
				break;
